...
} = require('./grade5-editorial-variant-audit')
...
function loadAudit() {
  if (!cachedAudit) cachedAudit = auditAllGrade5Variants()
  return cachedAudit
}

function validateDecision(decision, catalogItem, auditItem) {
  ...
  if (
    !Number.isInteger(decision.variantCount)
    || decision.variantCount <= 0
    || decision.variantCount !== auditItem.variantCount
  ) {
    throw new Error(`${reviewId}: variantCount must match the exhaustive audit`)
  }
  if (auditItem.issues.length > 0) {
    throw new Error(`${reviewId}: exhaustive variant audit has unresolved issues`)
  }
...
function createReceipt(decisions = loadDecisions()) {
  if (
    ...
    throw new Error('Grade 5 review decisions require schemaVersion 1 and an items array')
  }
  const catalog = buildCatalog(
    loadActualSources(ROOT_DIR).filter((source) => source.grade === 5),
    RENDERER_REVIEW_VERSION_REGISTRY,
  )
  const audit = loadAudit()
  if (audit.items.length !== 780 || audit.totalVariants !== 196_167) {
    throw new Error('Grade 5 exhaustive audit inventory changed unexpectedly')
  }
  const catalogById = new Map(catalog.items.map((item) => [item.reviewId, item]))
  const auditById = new Map(audit.items.map((item) => [
    `5:template:${item.templateId}`,
    item,
  ]))
  const decisionById = new Map()
  for (const decision of decisions.items) {
    if (!isRecord(decision) || !nonEmptyString(decision.reviewId)) {
      throw new Error('Grade 5 review decision reviewId is required')
    }
  ...
  const receipt = {
    ...
    items: catalog.items.map((item) => {
      ...
      const auditItem = auditById.get(item.reviewId)
      if (!auditItem) {
        throw new Error(`missing exhaustive variant audit: ${item.reviewId}`)
      }
      validateDecision(decision, item, auditItem)
...
if (require.main === module) {
  ...
  console.log(`Wrote Grade 5 editorial receipt to ${outputPath}`)
}
