...
const definitions: readonly G2FiniteDraftFamilyDefinition[] = [
  ...
  },
  {
    ...
    render: (p) => { const total = n(p, 'total'), first = n(p, 'first'), second = n(p, 'second'), answer = total - first - second; return { prompt: `세 종류를 조사한 수는 모두 ${total}개예요. 두 종류가 ${first}개와 ${second}개라면 나머지는 몇 개일까요?`, answer: { format: 'number', normalized: String(answer) }, solutionSteps: [`알려진 두 수는 ${first + second}개예요.`, `${total}-${first}-${second}=${answer}개`], hintSteps: ['전체에서 알려진 두 종류를 빼세요.', '세 수를 더해 전체인지 확인하세요.'] } },
  },
  {
  ...
  },
  {
    ...
    misconceptionRefs: ['table-graph-count-marks-not-scale'], visualSurface: 'diagram',
    visualValueKeys: (params) => n(params, 'has-key') === 1 ? ['marks', 'per-mark'] : ['marks'],
