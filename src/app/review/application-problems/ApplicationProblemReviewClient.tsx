    ...
    <article data-testid="review-problem-card" className="min-w-0 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="mt-3 grid gap-3 md:grid-cols-2">
        <DetailSection title="풀이">
          <ol className="list-decimal space-y-1 pl-5">{row.solutionSteps.map((step) => <li key={step}>{step}</li>)}</ol>
        </DetailSection>
        <DetailSection title="힌트">
          <ol className="list-decimal space-y-1 pl-5">{row.hintSteps.map((step) => <li key={step}>{step}</li>)}</ol>
        </DetailSection>
      </div>
