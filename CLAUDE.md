...
한국 초등학생이 가입 없이 바로 시작하는 비상업적 개인 수학 학습 웹앱이다. 현재 작업트리는 1·2·3학년 미션, 4학년 큰 수 단원, 5학년 개념별 10문제 연습, 6학년 비와 비율 5/10문제 학습을 제공한다.
2·5·6학년에는 프로젝트 책임자가 승인한 응용문제 9개를 기존 학습 흐름에 추가했으며, 장기 범위는 1~6학년 모든 단원이다. 정적 배포와 기기 로컬 기록을 기본으로 유지한다.
...
│           └── 0001-*.md … 0010-*.md → 선택, 대안, 이후 제약을 담은 결정 기록
├── public/
│   └── data/
