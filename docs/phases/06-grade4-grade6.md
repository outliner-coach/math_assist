...
현재 6A 원장은 92개 학년군 성취기준을 모두 배정했다. 4학년은
`[4수01-01]`, `[4수01-02]` 큰 수, `[4수01-07]` 두 자리 수로
나누기, `[4수01-08]` 사칙계산 어림, `[4수01-13]`·`[4수01-14]`
소수, `[4수01-15]` 분수의 덧셈과 뺄셈, `[4수01-16]` 소수의
덧셈과 뺄셈, `[4수02-01]`·`[4수02-02]` 규칙 찾기,
`[4수02-03]` 등호와 양의 관계, `[4수03-03]` 수직과 평행,
`[4수03-04]`·`[4수03-05]` 도형의 이동을 포함해 원장에 4학년으로
배정된 전 성취기준을 15개 단원으로 공개한다. 6학년은
`[6수01-10]`·`[6수01-11]` 분수의 나눗셈과
`[6수02-02]`·`[6수02-03]` 비와 비율, 두 단원을 작업트리에서
`released`로 유지한다. 구현·비활성 범위와 공개 게이트의 상세
계약은 [`docs/grade4-bridge-release-scope.md`](../grade4-bridge-release-scope.md)와
현재 원장에 있다. 4학년 배정 성취기준은 모두 `released`이며,
나머지 6학년 배정 성취기준은 `planned`다.
