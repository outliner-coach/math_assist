...
| 2. 문제 청사진 | 완료·콘텐츠 보강 진행 | 5학년 660개 모두 명시적 문제군·K/A/R 청사진과 의미 self-check를 통과한다. 발견된 9개는 문장·범위·solver·풀이를 함께 바로잡았다. 2026-07-25에는 반올림·올림과 버림·혼합 계산·소수의 곱셈·평균을 각각 K12/A12/R6의 10개 문제군으로 재설계했다. |
...
3. 5학년 응용문제 품질 경고 41개를 단원별로 줄인다. 다음 우선순위는 `unit-5-2-fraction-mul`의 `fracmul-001`이며 현재 K21/A9/R0, 문제군 5개다. 메타데이터 이름만 바꾸지 않고 문장·범위·solver·풀이를 함께 보강한다.
