...
다음 우선순위는 `unit-5-1-fraction-addsub`의 `fracadd-001`과
`fracsub-001`이다. 두 은행 모두 K18/A12/R0, 문제군 4개이므로 단원 전체
60문항을 실제 상황, 역산, 오답 분석 문제로 보강한다. 이후에도 한 단원씩
실패 테스트, 사람이 검토한 문제 구조, 생성 재현성, 전체 감사를 묶어
진행한다.
