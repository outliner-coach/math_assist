...
프로젝트 공통 지침은 `/AGENTS.md`를 기준으로 한다.
...
```bash
npm run tdd:guard # 제품 코드 변경 시 테스트 변경이 함께 있는지 확인
...
2. 아키텍처 체크리스트를 확인한다:
   ...
   - AGENTS.md 규칙을 위반하지 않았는가?
   - 제품 코드 변경 시 테스트 파일을 함께 추가/수정했는가?
...
- 제품 코드만 수정하고 테스트를 추가/수정하지 마라. 이유: `.codex/hooks/tdd-guard.sh`가 해당 변경을 거부한다.
- 기존 테스트를 깨뜨리지 마라
```
...
- `codex/{task-name}` 브랜치 생성/checkout
...
- Codex hook 실행 — `.codex/hooks/preflight.sh`로 실행 전 점검, `.codex/hooks/tdd-guard.sh`로 테스트 없는 제품 코드 변경 차단, `.codex/hooks/post-step.sh`로 lint/build/test 검증
